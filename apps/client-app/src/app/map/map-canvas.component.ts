  ...
  implements AfterViewInit, OnChanges, OnDestroy
{
  ...
  private readonly gridLayer = L.layerGroup();
  ...
  private renderGrid(): void {
    ...
    for (let position = DEMO_MAP_GRID_STEP; position < maxX; position += DEMO_MAP_GRID_STEP) {
      L.polyline(
        [
          [minY, position],
          [maxY, position],
  ...
  private renderLayers(): void {
    ...
    this.regions?.features.forEach((feature) => {
      L.polygon(feature.points.map((point) => this.toLatLng(point)), {
        ...
        weight: feature.status === 'critical' ? 3 : 2,
      })
